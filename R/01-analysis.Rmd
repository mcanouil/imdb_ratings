...
theme_set(theme_black(11, base_family = "xkcd") + theme(plot.title.position = "plot", plot.caption.position = "plot"))
...
```{r ratings, fig.width = 16 / 2.54, fig.height = 24.7 / 2.54}
who_colours <- setNames(viridis_pal(begin = 0.5)(3), toupper(c("Coeos", "metascore", "imdb")))
...
p_ratings <- ggplot(
  data = pivot_longer(
    ...
    mutate(
      who = factor(
        x = who, 
        levels = c(COEOS = "your", METASCORE = "metascore", IMDB = "imdb")[names(who_colours)], 
        labels = names(who_colours)
      )
    ),
  mapping = aes(
    x = rating, 
    ...
    colour = who, 
    fill = who,
    group = paste(year_rated, who, sep = "_")
  )
) +
  geom_bar(
  ...
  scale_x_binned(
    ...
    # labels = function(x) ifelse(x %in% 10, x, glue("[{x},{x+1})")),
    expand = c(0, 0)
  ) +
  scale_y_continuous(labels = percent_format(suffix = " %"), expand = expansion(c(0, 0.25))) +
  scale_colour_manual(values = who_colours) +
  scale_fill_manual(values = who_colours) +
  labs(
    x = "Rating", 
    y = "Percentage of Movies", 
    colour = NULL,
    fill = NULL,
    title = glue_collapse(c(
      glue('Distribution of Ratings ({comma(nrow(drop_na(select(ratings_ms, ends_with("rating")))))})'),
      glue_collapse(
...
p_ratings + facet_grid(rows = vars(year_rated))

# p_ratings +
#   transition_states(year_rated) +
#       shadow_wake(wake_length = 1 / 3, wrap = TRUE)
