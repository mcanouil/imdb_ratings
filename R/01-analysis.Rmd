...
```{r ratings}
who_colours <- setNames(viridis_pal(begin = 0.5)(3), c("COEOS", "METASCORE", "IMDB"))
...
p_ratings <- ggplot(
  ...
  mapping = aes(
    ...
    colour = who, 
    fill = who,
    group = paste(who, year_rated, sep = "_")
  )
) +
  annotation_raster(
  ...
  labs(
    ...
    fill = NULL,
    title = glue(
      'Distribution of Ratings (N = {comma(nrow(drop_na(select(ratings_ms, ends_with("rating")))))})',
      text_legend,
      .sep = ": "
    ),
    caption = fig_caption
