...
p <- ggplot(data = data_radar) +
  ...
  # real geom
  geom_smooth(
    aes(x = as.integer(Month), y = Count, colour = "SMOOTH"),
    method = "loess",
    se = FALSE,
    size = 1,
    linetype = 2,
  ...
  geom_point(
    mapping = aes(x = as.integer(Month), y = Count, colour = Year, fill = Year), 
    size = 3, shape = 21
  ) +
  geom_path(
    mapping = aes(x = as.integer(Month), y = Count, colour = Year), 
    size = 1.5
  ) +
...
p <- ggplot(data = data_ratings, aes(x = rounded_rating, fill = Who)) +
  # geom_density(
  #   aes(x = rounded_rating, y = ..density..),
  #   bw = 1,
  #   alpha = 0.75,
  #   colour = "white"
  # ) +
