...
plot_ratings_distribution <- function(.data) {
  .data %>% 
    select(ends_with("Rating"), Year) %>%
    gather(key = Who, value = Rating, -Year) %>%
    mutate(
...
print(p, width = 500, height = 500, units = "px", bg = ggplot2::theme_get()$plot.background$colour)
anim_save(filename = "./images/Coeos_IMDb.gif")
```
...
ratings %>% 
  ...
  scale_y_continuous(expand = c(0, 0), labels = percent, limits = c(0, 0.15)) +
  scale_fill_viridis_d() +
  labs(
    x = "Rating", 
    y = "Proportion", 
  ...
  exit_disappear() +
  ease_aes('linear') + 
  labs(title = "Distribution of Ratings: {closest_state}")

print(p, width = 500, height = 175, units = "px", bg = ggplot2::theme_get()$plot.background$colour)
