...
if (params$theme_dark) {
  theme_set(theme_black(base_size = params$gg_fontsize, base_family = base_family))
} else {
  ggplot2::theme_set(ggplot2::theme_grey(base_size = params$gg_fontsize, base_family = base_family))
}
```
...
gg_ratings <- ratings %>% 
  ...
  dplyr::mutate(
    ...
    p_bad = purrr::map(
      .x = data, 
      base_family = base_family,
      .f = ~plot_bad(.data = .x, base_family = base_family)
    ),
    p_top = purrr::map(
      .x = data, 
      base_family = base_family,
      .f = ~plot_top(.data = .x, base_family = base_family)
    )
  ) %>% 
  dplyr::mutate(
...
# GIF {.tabset}
```{r data_radar}
range_dates <- min(movies_theatres[["Year"]]):max(movies_theatres[["Year"]])
p <- movies_theatres %>% 
  ...
  ggplot2::scale_colour_manual(
    name = "Year",
    values = c("white", scales::viridis_pal(direction = 1)(length(range_dates))) %>% 
